...
$cacheRoot = $projectRoot . DIRECTORY_SEPARATOR . '.clone-source' . DIRECTORY_SEPARATOR . 'pages';
...
foreach (file($missingUrlsPath, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES) ?: [] as $url) {
    $path = normalizePath((string) parse_url(trim($url), PHP_URL_PATH));
    ...
    $paths[$path] = true;
    $cachePath = pageCachePath($cacheRoot, $path);

    if (! is_file($cachePath)) {
        fwrite(STDERR, "Missing cached HTML for {$path}" . PHP_EOL);
        continue;
    }
    ...
    if (! is_string($html) || $html === '') {
        fwrite(STDERR, "Empty cached HTML for {$path}" . PHP_EOL);
        continue;
    }

    $pageData = transformPageHtml($html, $baseUrl, $publicAssetRoot, $assets, $skipPrefixes);

    $pages[] = [
        'path' => $path,
        'view' => viewNameForPath($path),
    ...
    echo "Prepared page from cache: {$path}" . PHP_EOL;
}
...
echo 'Generated ' . count($pages) . ' cached Blade pages and checked ' . count($assets) . ' asset references.' . PHP_EOL;
...
function pageCachePath(string $cacheRoot, string $path): string
{
    $slug = trim($path, '/');
    $slug = $slug === '' ? 'home' : str_replace('/', '__', $slug);

    return $cacheRoot . DIRECTORY_SEPARATOR . $slug . '.html';
}
...
function transformPageHtml(
    ...
    foreach ($xpath->query('//*[@href]') as $node) {
        rewriteUrlAttribute($node, 'href', $baseUrl, $publicAssetRoot, $assets, $skipPrefixes);
    }

    foreach ($xpath->query('//*[@src]') as $node) {
        rewriteUrlAttribute($node, 'src', $baseUrl, $publicAssetRoot, $assets, $skipPrefixes);
    }
